...
/* Functions to create basic immutable strings.
 * The provided allocator is used for all memory activity in these functions.
 */

/* The following four functions copy the provided buffer 
 * into CFString's internal storage.
 */
...
/* The following takes an explicit length, and allows you to specify
 * whether the data is an external format --- that is, whether to pay 
 * attention to the BOM character (if any) and do byte swapping if necessary
 */
CF_EXPORT
 ...
 *
 * "maxLength", if not 0, is a hard bound on the length of the string. If 0, 
...
/* This function creates a mutable string that has a developer supplied 
 * and directly editable backing store.
 * The string will be manipulated within the provided buffer (if any) until 
 * it outgrows capacity; then the externalCharactersAllocator will be consulted 
 * for more memory. When the CFString is deallocated, the buffer will be freed 
 * with the externalCharactersAllocator. Provide kCFAllocatorNull here to prevent 
 * the buffer from ever being reallocated or deallocated by CFString. See comments 
 * at top of this file for more info.
 */
CF_EXPORT
...
/* The default encoding for the system; untagged 8-bit characters 
 * are usually in this encoding.
 */
CF_EXPORT
 ...
 * for passing to POSIX APIs (for example, normalized for HFS+).
 * The string is zero-terminated.
 * Returns false if the conversion results don't fit into the buffer.
 * Use CFStringGetMaximumSizeOfFileSystemRepresentation() if you want to make sure 
 * the buffer is of sufficient length.
 */
CF_EXPORT
 ...
 * This result is returned quickly as a very rough approximation, and could be 
 * much larger than the actual space required. The result includes space for 
 * the zero termination. If you are allocating a buffer for long-term keeping,
 * it's recommended that you reallocate it smaller (to be the right size) after 
 * calling CFStringGetFileSystemRepresentation().
 */
CF_EXPORT
