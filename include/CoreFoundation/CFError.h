 ...
 *  @header CFError
 *  @discussion
 *  CFErrors are used to encompass information about errors. At minimum, errors are 
 *   identified by their domain (a string) and an error code within that domain. In 
 *   addition a "userInfo" dictionary supplied at creation time enables providing 
 *   additional info that might be useful for the interpretation and reporting of 
 *   the error. This dictionary can even contain an "underlying" error, which is 
 *   wrapped as an error bubbles up through various layers.
 *
 *  CFErrors have the ability to provide human-readable descriptions for the errors; 
 *   in fact, they are designed to provide localizable, end-user presentable errors 
 *   that can appear in the UI. CFError has a number of predefined userInfo keys 
 *   to enable developers to supply the info.
 *
 *  Usage recommendation for CFErrors is to return them as by-ref parameters in 
 *   functions. This enables the caller to pass NULL in when they don't actually 
 *   want information about the error. The presence of an error should be reported 
 *   by other means, for instance a NULL or false return value from the function 
 *   call proper:
 *
 *   CFError *error;
 ...
 *
 *  In addition, it's recommended that CFErrors be used in error situations only 
 *   (not status), and where there are multiple possible errors to distinguish between.
 *   For instance there is no plan to add CFErrors to existing APIs in CF which currently
 *   don't return errors; in many cases, there is one possible reason for failure, and a
 *   false or NULL return is enough to indicate it.
 *
 *  CFError is toll-free bridged to NSError in Foundation. NSError in Foundation has some 
 *   additional guidelines which makes it easy to automatically report errors to users 
 *   and even try to recover from them.
 */
...
/*!
 *  @typedef CFErrorRef
 *  This is the type of a reference to CFErrors.
 */
...
/* Predefined domains; value of "code" will correspond to preexisting
...
/* Keys in userInfo for localizable, end-user presentable error messages.
 * At minimum provide one of first two; ideally provide all three.
 */
...
// Key to identify the end user-presentable failure reason in userInfo.
...
// If you do not have localizable error strings, you can provide a value 
//  for this key instead.
// Should be a complete sentence if possible.
// Should not contain domain name or error code.
CF_EXPORT const CFStringRef kCFErrorDescriptionKey;

// A key for "true" debugging descriptions which should never be shown to the user.
CF_EXPORT const CFStringRef kCFErrorDebugDescription;
...
/* This callback function is consulted if a key is not present in the userInfo dictionary. 
 * Note that setting a callback for the same domain again simply replaces the previous callback.
 * Set NULL as the callback to remove it.
 */
typedef CFTypeRef (*CFErrorUserInfoKeyCallBack)(CFErrorRef err, CFStringRef key);
CF_EXPORT void CFErrorSetCallBackForDomain(CFStringRef domainName, CFErrorUserInfoKeyCallBack callBack);
CF_EXPORT CFErrorUserInfoKeyCallBack CFErrorGetCallBackForDomain(CFStringRef domainName);
