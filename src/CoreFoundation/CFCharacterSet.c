...
/* TODO
 *
 * Make __CFCSetBitmapBits return 'const UInt32*' and get rid of casts.
 * Make functions from __CFCSetAnnexBitmapXXX macros that accept 
 *  CFCharacterSetRef instead of bitmap.
 * Cleanup mess like ((unsigned)0xFF) << (firstChar & (BITSPERBYTE - 1)))
 *  - extract inline functions.
 * Wrap long lines (at 100).
 * Use _CFBSearch (from Utilities) in __CFCSetBsearchUniChar.
 * Report type ID in "Internal inconsistency error: unknown character set type"
 * CFCSET_VALIDATE_RANGE should raise range error?
 * Rename CFCSET macros
 */

/* Validation funcs
 */
#define CFCSET_VALIDATE_BUILTIN_TYPE(type) \
    CF_VALIDATE_ARG(type > 0 && type <= __kCFLastBuiltinSetID, \
        "invalid builtin type %d", type);

#define CFCSET_VALIDATE_RANGE(theRange) \
    CF_VALIDATE_ARG(theRange.location >= 0 && theRange.location + theRange.length <= 0x1FFFFF, \
    "range {%d, %d} is out of Unicode range", theRange.location, theRange.length);

#define CF_VALIDATE_CSET_ARG(cset) \
    CF_VALIDATE_OBJECT_ARG(CFObjC, cset, __kCFCharacterSetTypeID)

#define CF_VALIDATE_MUTABLECSET_ARG(cset) \
    CF_VALIDATE_MUTABLEOBJECT_ARG(CFObjC, cset, __kCFCharacterSetTypeID, \
        __CFCSetIsMutable(cset))

#define CF_REPORT_INVALID_CSET_TYPE(cset) \
	CF_GENERIC_ERROR( \
		"Internal inconsistency error: unknown character set type %d", \
...
/* CF_INFO values interesting for CFCharacterSet
 */
enum {
...
/* Array of instantiated builtin set.
 * Note builtin set ID starts with 1 so the array index is ID - 1.
 */
static CFCharacterSetRef * __CFBuiltinSets = NULL;
static CFSpinLock_t __CFBuiltinSetsLock = CFSpinLockInit;
...
/* Inline info accessor macros
 */
CF_INLINE Boolean __CFCSetIsMutable(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetIsMutableMask) == __kCFCharSetIsMutable;
}

CF_INLINE Boolean __CFCSetIsBuiltin(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask) == __kCFCharSetClassBuiltin;
}

CF_INLINE Boolean __CFCSetIsRange(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask) == __kCFCharSetClassRange;
}

CF_INLINE Boolean __CFCSetIsString(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask) == __kCFCharSetClassString;
}

CF_INLINE Boolean __CFCSetIsBitmap(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask) == __kCFCharSetClassBitmap;
}

CF_INLINE Boolean __CFCSetIsCompactBitmap(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask) == __kCFCharSetClassCompactBitmap;
}

CF_INLINE Boolean __CFCSetIsInverted(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetIsInvertedMask) == __kCFCharSetIsInverted;
}

CF_INLINE Boolean __CFCSetHasHashValue(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetHasHashValueMask) == __kCFCharSetHasHashValue;
}

CF_INLINE UInt32 __CFCSetClassType(CFCharacterSetRef cset) {
    return (CF_INFO(cset) & __kCFCharSetClassTypeMask);
}

CF_INLINE void __CFCSetPutIsMutable(CFMutableCharacterSetRef cset, Boolean isMutable) {
    (isMutable ? (CF_INFO(cset) |= __kCFCharSetIsMutable) : (CF_INFO(cset) &= ~__kCFCharSetIsMutable));
}

CF_INLINE void __CFCSetPutIsInverted(CFMutableCharacterSetRef cset, Boolean isInverted) {
    (isInverted ? (CF_INFO(cset) |= __kCFCharSetIsInverted) : (CF_INFO(cset) &= ~__kCFCharSetIsInverted));
}

CF_INLINE void __CFCSetPutHasHashValue(CFMutableCharacterSetRef cset, Boolean hasHash) {
    (hasHash ? (CF_INFO(cset) |= __kCFCharSetHasHashValue) : (CF_INFO(cset) &= ~__kCFCharSetHasHashValue));
}

CF_INLINE void __CFCSetPutClassType(CFMutableCharacterSetRef cset, UInt32 classType) {
    CF_INFO(cset) &= ~__kCFCharSetClassTypeMask;
    CF_INFO(cset) |= classType;
}

/* Inline contents accessor macros
 */
CF_INLINE CFCharacterSetPredefinedSet __CFCSetBuiltinType(CFCharacterSetRef cset) {
    return cset->_variants._builtin._type;
}

CF_INLINE UInt32 __CFCSetRangeFirstChar(CFCharacterSetRef cset) {
    return cset->_variants._range._firstChar;
}

CF_INLINE CFIndex __CFCSetRangeLength(CFCharacterSetRef cset) {
    return cset->_variants._range._length;
}

CF_INLINE UniChar* __CFCSetStringBuffer(CFCharacterSetRef cset) {
    return (UniChar*)(cset->_variants._string._buffer);
}

CF_INLINE CFIndex __CFCSetStringLength(CFCharacterSetRef cset) {
    return cset->_variants._string._length;
}

CF_INLINE uint8_t* __CFCSetBitmapBits(CFCharacterSetRef cset) {
    return cset->_variants._bitmap._bits;
}

CF_INLINE uint8_t* __CFCSetCompactBitmapBits(CFCharacterSetRef cset) {
    return cset->_variants._compactBitmap._cBits;
}

CF_INLINE void __CFCSetPutBuiltinType(CFMutableCharacterSetRef cset, CFCharacterSetPredefinedSet type) {
    cset->_variants._builtin._type = type;
}

CF_INLINE void __CFCSetPutRangeFirstChar(CFMutableCharacterSetRef cset, UInt32 first) {
    cset->_variants._range._firstChar = first;
}

CF_INLINE void __CFCSetPutRangeLength(CFMutableCharacterSetRef cset, CFIndex length) {
    cset->_variants._range._length = length;
}

CF_INLINE void __CFCSetPutStringBuffer(CFMutableCharacterSetRef cset, UniChar* theBuffer) {
    cset->_variants._string._buffer = theBuffer;
}

CF_INLINE void __CFCSetPutStringLength(CFMutableCharacterSetRef cset, CFIndex length) {
    cset->_variants._string._length = length;
}

CF_INLINE void __CFCSetPutBitmapBits(CFMutableCharacterSetRef cset, uint8_t* bits) {
    cset->_variants._bitmap._bits = bits;
}

CF_INLINE void __CFCSetPutCompactBitmapBits(CFMutableCharacterSetRef cset, uint8_t* bits) {
    cset->_variants._compactBitmap._cBits = bits;
}
...
static Boolean __CFCSetIsBitmapEqualToRange(const UInt32* bits, UniChar firstChar, UniChar lastChar, Boolean isInverted) {
    ...
    if (firstCharIndex == lastCharIndex) {
        ...
        if (*(bits + firstCharIndex) != (isInverted ? ~value : value)) {
            return FALSE;
        }
    } else {
        ...
        if (firstCharIndex == lastCharIndex) {
            ...
            if (*(bits + lastCharIndex) != (isInverted ? ~value : value)) {
                return FALSE;
            }
        }
    }
...
CF_INLINE Boolean __CFCSetIsBitmapSupersetOfBitmap(const UInt32* bits1, const UInt32* bits2,
                                                   Boolean isInverted1, Boolean isInverted2)
{
    CFIndex length = __kCFBitmapSize / sizeof(UInt32);
    UInt32 val1, val2;
...
CF_INLINE Boolean __CFCSetHasNonBMPPlane(CFCharacterSetRef cset) {
    return ((cset)->_annex && (cset)->_annex->_validEntriesBitmap ? true : false);
}

CF_INLINE Boolean __CFCSetAnnexIsInverted(CFCharacterSetRef cset) {
    return ((cset)->_annex && (cset)->_annex->_isAnnexInverted ? true : false);
}

CF_INLINE UInt32 __CFCSetAnnexValidEntriesBitmap(CFCharacterSetRef cset) {
    return ((cset)->_annex ? (cset)->_annex->_validEntriesBitmap : 0);
}

CF_INLINE Boolean __CFCSetIsEmpty(CFCharacterSetRef cset) {
    if (__CFCSetHasNonBMPPlane(cset) || __CFCSetAnnexIsInverted(cset)) {
        return false;
    }
    switch (__CFCSetClassType(cset)) {
        case __kCFCharSetClassRange: return !__CFCSetRangeLength(cset);
        case __kCFCharSetClassString: return !__CFCSetStringLength(cset);
        case __kCFCharSetClassBitmap: return !__CFCSetBitmapBits(cset);
        case __kCFCharSetClassCompactBitmap: return !__CFCSetCompactBitmapBits(cset);
    }
    return false;
}
...
CF_INLINE void __CFCSetAnnexSetIsInverted(CFCharacterSetRef cset, Boolean flag) {
    if (flag) {
        __CFCSetAllocateAnnexForPlane(cset, 0);
    }
    if (cset->_annex) {
        ((CFMutableCharacterSetRef)cset)->_annex->_isAnnexInverted = flag;
    }
}
...
CF_INLINE void __CFCSetDeallocateAnnexPlane(CFCharacterSetRef cset) {
    if (cset->_annex) {
        ...
        for (idx = 0; idx < __kCFMaxAnnexPlane; idx++) {
...
CF_INLINE uint32_t __CFCSetGetCompactBitmapSize(const uint8_t* compactBitmap) {
    ...
    while (length-- > 0) {
        value = *(compactBitmap++);
        if ((value != 0) && (value != UINT8_MAX)) {
            size += __kCFCompactBitmapPageSize;
        }
    }
    return size;
}
...
static void __CFCSetGetBitmap(CFCharacterSetRef cset, uint8_t* bits) {
    ...
    if (__CFCSetIsBitmap(cset) && (bitmap = __CFCSetBitmapBits(cset))) {
        ...
        bitmap = bits;
        while (length--) {
            *bitmap++ = value; // Initialize the buffer

        }
        if (!__CFCSetIsEmpty(cset)) {
            switch (__CFCSetClassType(cset)) {
                case __kCFCharSetClassBuiltin: {
                    UInt8 result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(cset), 0, bits, (isInverted != 0));
                ...
                case __kCFCharSetClassRange: {
                    ...
                    if (theChar < NUMCHARACTERS) { // the range starts in BMP
                        ...
                        if (theChar + length >= NUMCHARACTERS) {
                            length = NUMCHARACTERS - theChar;
                        }
                        if (isInverted) {
                ...
                case __kCFCharSetClassString: {
                    ...
                    while (length--) {
                        (isInverted ? __CFCSetBitmapRemoveCharacter(bits, *buffer++) : __CFCSetBitmapAddCharacter(bits, *buffer++));
                    }
                }
                break;
...
static Boolean __CFCSetIsEqual(CFCharacterSetRef cf1, CFCharacterSetRef cf2) {
    Boolean isInvertStateIdentical = (__CFCSetIsInverted(cf1) == __CFCSetIsInverted(cf2));
    Boolean isAnnexInvertStateIdentical = (__CFCSetAnnexIsInverted(cf1) == __CFCSetAnnexIsInverted(cf2));
    CFIndex idx;
    ...
    if (__CFCSetHasHashValue(cf1) && __CFCSetHasHashValue(cf2) && (cf1)->_hashValue != (cf2)->_hashValue) {
        return false;
    }
    if (__CFCSetIsEmpty(cf1) && __CFCSetIsEmpty(cf2) && !isInvertStateIdentical) {
        return false;
    }
    ...
    if (__CFCSetClassType(cf1) == __CFCSetClassType(cf2)) { // Types are identical, we can do it fast
        switch (__CFCSetClassType(cf1)) {
            ...
            case __kCFCharSetClassBitmap:
                if (!__CFCSetIsEqualBitmap((const UInt32*)__CFCSetBitmapBits(cf1), (const UInt32*)__CFCSetBitmapBits(cf2))) {
                    return false;
                }
                break;
        }
    ...
    if (__CFCSetIsEmpty(cf1) || __CFCSetIsEmpty(cf2)) {
        ...
        if (__CFCSetIsBuiltin(nonEmptySet)) {
            ...
            if (__CFCSetAnnexIsInverted(nonEmptySet)) {
                if (__CFCSetAnnexValidEntriesBitmap(nonEmptySet) != 0x1FFFE) {
                    return false;
                }
            } else {
                if (__CFCSetAnnexValidEntriesBitmap(nonEmptySet)) {
                    return false;
                }
            }
            ...
            for (idx = 1; idx < __kCFMaxAnnexPlane; idx++) {
                if (__CFCSetIsBitmap(nonEmptySet)) {
                    if (!__CFCSetIsEqualBitmap((__CFCSetAnnexIsInverted(nonEmptySet) ? NULL : (const UInt32*)-1), (const UInt32*)bitsBuf)) {
                        return false;
                    }
                } else {
                    __CFCSetGetBitmap(__CFCSetGetAnnexPlaneCharacterSetNoAlloc(nonEmptySet, idx), bitsBuf);
                    if (!__CFCSetIsEqualBitmap((const UInt32*)-1, (const UInt32*)bitsBuf)) {
                        return false;
                    }
                }
            }
    ...
    if (__CFCSetIsBuiltin(cf1) || __CFCSetIsBuiltin(cf2)) {
        ...
        if (__CFCSetIsRange(nonBuiltinSet)) {
            ...
            for (idx = 0; idx < __kCFMaxAnnexPlane; idx++) {
                result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(builtinSet), idx, bitsBuf, (isInvertStateIdentical != 0));

                if (idx < firstPlane || idx > lastPlane) {
                    if (result == kCFUniCharBitmapAll) {
                        return false;
                    } else if (result == kCFUniCharBitmapFilled) {
                        if (!__CFCSetIsEqualBitmap(NULL, (const UInt32*)bitsBuf)) {
                            return false;
                        }
                    }
                    ...
                    if (result == kCFUniCharBitmapEmpty) {
                        ...
                        if (idx == firstPlane) {
                            if (((firstChar & 0xFFFF) != 0) || (firstPlane == lastPlane && ((lastChar & 0xFFFF) != 0xFFFF))) {
                                return false;
                            }
                        } else {
                            if (((lastChar & 0xFFFF) != 0xFFFF) || (firstPlane == lastPlane && ((firstChar & 0xFFFF) != 0))) {
                                return false;
                            }
                        }
                    } else {
                        if (idx == firstPlane) {
                            if (!__CFCSetIsBitmapEqualToRange((const UInt32*)bitsBuf, firstChar & 0xFFFF, (firstPlane == lastPlane ? lastChar & 0xFFFF : 0xFFFF), false)) {
                                return false;
                            }
                        } else {
                            if (!__CFCSetIsBitmapEqualToRange((const UInt32*)bitsBuf, (firstPlane == lastPlane ? firstChar & 0xFFFF : 0), lastChar & 0xFFFF, false)) {
                                return false;
                            }
                        }
                    }
            ...
            result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(builtinSet), 0, bitsBuf, (__CFCSetIsInverted(builtinSet) != 0));
            if (result == kCFUniCharBitmapFilled) {
                if (__CFCSetIsBitmap(nonBuiltinSet)) {
                    if (!__CFCSetIsEqualBitmap((const UInt32*)bitsBuf, (const UInt32*)__CFCSetBitmapBits(nonBuiltinSet))) {
                        return false;
                    }
                } else {
                ...
                if (__CFCSetIsBitmap(nonBuiltinSet)) {
                    if (!__CFCSetIsEqualBitmap((result == kCFUniCharBitmapAll ? (const UInt32*)-1 : NULL), (const UInt32*)__CFCSetBitmapBits(nonBuiltinSet))) {
                        return false;
                    }
                } else {
            ...
            for (idx = 1; idx < __kCFMaxAnnexPlane; idx++) {
                result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(builtinSet), idx, bitsBuf, !isInvertStateIdentical);
    ...
    if (__CFCSetIsRange(cf1) || __CFCSetIsRange(cf2)) {
        ...
        if (firstPlane == 0) {
            if (!__CFCSetIsBitmapEqualToRange((const UInt32*)bits, firstChar, (lastPlane == 0 ? lastChar : 0xFFFF), isRangeSetInverted)) {
                return false;
            }
            firstPlane = 1;
            firstChar = 0;
        } else {
            if (!__CFCSetIsEqualBitmap((const UInt32*)bits, (isRangeSetInverted ? (const UInt32*)-1 : NULL))) {
                return false;
            }
            firstChar &= 0xFFFF;
        }
        ...
        for (idx = 1; idx < __kCFMaxAnnexPlane; idx++) {
            ...
            if (!subSet1) {
                if (idx < firstPlane || idx > lastPlane) {
                    ...
                    if (isAnnexInvertStateIdentical || firstChar || (idx == lastPlane && lastChar != 0xFFFF)) {
                        return false;
                    }
                } else if (idx == lastPlane) {
                    if (isAnnexInvertStateIdentical || (idx == firstPlane && firstChar) || (lastChar != 0xFFFF)) {
                        return false;
                    }
                }
            } else {
                ...
                if (idx < firstPlane || idx > lastPlane) {
                    if (!__CFCSetIsEqualBitmap((const UInt32*)bits, (isAnnexInvertStateIdentical ? NULL : (const UInt32*)-1))) {
                        return false;
                    }
                } else if (idx > firstPlane && idx < lastPlane) {
                    if (!__CFCSetIsEqualBitmap((const UInt32*)bits, (isAnnexInvertStateIdentical ? (const UInt32*)-1 : NULL))) {
                        return false;
                    }
                } else if (idx == firstPlane) {
                    if (!__CFCSetIsBitmapEqualToRange((const UInt32*)bits, firstChar, (idx == lastPlane ? lastChar : 0xFFFF), !isAnnexInvertStateIdentical)) {
                        return false;
                    }
                } else if (idx == lastPlane) {
                    if (!__CFCSetIsBitmapEqualToRange((const UInt32*)bits, (idx == firstPlane ? firstChar : 0), lastChar, !isAnnexInvertStateIdentical)) {
                        return false;
                    }
                }
            }
    ...
    if (isBitmap1 && isBitmap2) {
        ...
        if (!__CFCSetIsEqualBitmap((const UInt32*)__CFCSetBitmapBits(cf1), (const UInt32*)bitsBuf)) {
            return false;
        }
    }
    return __CFCSetIsEqualAnnex(cf1, cf2);
...
static Boolean __CFCSetIsEqualAnnex(CFCharacterSetRef cf1, CFCharacterSetRef cf2) {
    ...
    if (isAnnexInvertStateIdentical) {
        if (__CFCSetAnnexValidEntriesBitmap(cf1) != __CFCSetAnnexValidEntriesBitmap(cf2)) {
            return false;
        }
        for (idx = 1; idx <= __kCFMaxAnnexPlane; idx++) {
            ...
            if (subSet1 && !__CFCSetIsEqual(subSet1, subSet2)) {
                return false;
            }
        }
    } else {
...
static uint8_t* __CFCreateCompactBitmap(CFAllocatorRef allocator, const uint8_t* bitmap) {
    ...
    for (i = 0; i < __kCFCompactBitmapNumPages; i++) {
        ...
        if (numPages > __kCFCompactBitmapMaxPages) {
            return NULL;
        }
...
static void __CFCSetAddNonBMPPlanesInRange(CFMutableCharacterSetRef cset, CFRange range) {
    ...
    for (idx = (idx ? idx : 1); idx <= maxPlane; idx++) {
        planeRange.location = _CFMax(firstChar, 0);
...
static void __CFCSetMakeBitmap(CFMutableCharacterSetRef cset) {
    if (!__CFCSetIsBitmap(cset) || !__CFCSetBitmapBits(cset)) {
        ...
        if (__CFCSetIsBuiltin(cset)) {
            CFIndex numPlanes = _CFUniCharGetNumberOfPlanes(__CFCSetBuiltinType(cset));
            ...
            if (numPlanes > 1) {
                ...
                for (idx = 1; idx < numPlanes; idx++) {
                    ...
                    result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(cset), idx, annexBitmap, false);
                    if (result == kCFUniCharBitmapEmpty) {
                        continue;
                    }
                    if (result == kCFUniCharBitmapAll) {
                ...
                if (annexBitmap) {
                    CFAllocatorDeallocate(allocator, annexBitmap);
                }
            }
...
/* Binary search theChar for __kCFCharSetClassString.
 * Character set must have been be sorted by __CFCSetSortUniChars.
 */
CF_INLINE Boolean __CFCSetBsearchUniChar(CFCharacterSetRef cset, UniChar theChar) {
    const UniChar* p, * q, * divider;

    const UniChar* theTable = __CFCSetStringBuffer(cset);
    CFIndex length = __CFCSetStringLength(cset);

    if (!length || (theChar < theTable[0]) || (theChar > theTable[length - 1])) {
        return false;
    }
...
static int __CFCSetSortUniCharsFunc(const void* a, const void* b) {
    return -(int)(*(UniChar*)b - *(UniChar*)a);
}
CF_INLINE void __CFCSetSortUniChars(CFMutableCharacterSetRef set) {
    qsort(__CFCSetStringBuffer(set), __CFCSetStringLength(set), sizeof(UniChar), __CFCSetSortUniCharsFunc);
}

CF_INLINE CFCharacterSetRef __CFCharacterSetGetExpandedSetForNSCharacterSet(const void* characterSet) {
    CF_OBJC_FUNCDISPATCH(CFCharacterSetRef, characterSet, "_expandedCFCharacterSet");
    return NULL;
}
...
static CFMutableCharacterSetRef __CFCharacterSetCreateCopy(CFAllocatorRef alloc, CFCharacterSetRef theSet, bool isMutable) {
    ...
    CF_OBJC_FUNCDISPATCH(CFMutableCharacterSetRef, theSet, "mutableCopy");

    CF_VALIDATE_CSET_ARG(theSet);
    ...
    switch (__CFCSetClassType(theSet)) {
        ...
        default:
            CF_REPORT_INVALID_CSET_TYPE(theSet);
    }
...
/*** CFCharacterSet class ***/

static Boolean __CFCharacterSetEqual(CFTypeRef cf1, CFTypeRef cf2) {
    return __CFCSetIsEqual((CFCharacterSetRef)cf1, (CFCharacterSetRef)cf2);
}
...
static CFHashCode __CFCharacterSetHash(CFTypeRef cf) {
    if (!__CFCSetHasHashValue((CFCharacterSetRef)cf)) {
        if (__CFCSetIsEmpty((CFCharacterSetRef)cf)) {
            ...
            ((CFMutableCharacterSetRef)cf)->_hashValue = _CFHashBytes(__CFCSetBitmapBits((CFCharacterSetRef)cf), __kCFBitmapSize);
        } else {
            uint8_t bitsBuf[__kCFBitmapSize];
            __CFCSetGetBitmap((CFCharacterSetRef)cf, bitsBuf);
            ((CFMutableCharacterSetRef)cf)->_hashValue = _CFHashBytes(bitsBuf, __kCFBitmapSize);
        }
...
static CFStringRef  __CFCharacterSetCopyDescription(CFTypeRef cf) {
    ...
    CF_REPORT_INVALID_CSET_TYPE((CFCharacterSetRef)cf);
    return NULL;
}
...
static void __CFCharacterSetDeallocate(CFTypeRef cf) {
    ...
    if (__CFCSetIsBuiltin((CFCharacterSetRef)cf) && !__CFCSetIsMutable((CFCharacterSetRef)cf) && !__CFCSetIsInverted((CFCharacterSetRef)cf)) {
        ...
        if (sharedSet == cf) {
            ...
            CF_GENERIC_ERROR("predefined character set objects can't be deallocated");
            return;
        }
    }

    if (__CFCSetIsString((CFCharacterSetRef)cf) && __CFCSetStringBuffer((CFCharacterSetRef)cf)) {
        CFAllocatorDeallocate(allocator, __CFCSetStringBuffer((CFCharacterSetRef)cf));
    } else if (__CFCSetIsBitmap((CFCharacterSetRef)cf) && __CFCSetBitmapBits((CFCharacterSetRef)cf)) {
        CFAllocatorDeallocate(allocator, __CFCSetBitmapBits((CFCharacterSetRef)cf));
    } else if (__CFCSetIsCompactBitmap((CFCharacterSetRef)cf) && __CFCSetCompactBitmapBits((CFCharacterSetRef)cf)) {
        CFAllocatorDeallocate(allocator, __CFCSetCompactBitmapBits((CFCharacterSetRef)cf));
    }
...
CF_INTERNAL void _CFCharacterSetInitialize(void) {
    const char* checkForExpandedSet = getenv("__CF_DEBUG_EXPANDED_SET");
    if (checkForExpandedSet && (*checkForExpandedSet == 'Y')) {
        __CFCheckForExapendedSet = true;
    }
...
CFCharacterSetRef CFCharacterSetGetPredefined(CFCharacterSetPredefinedSet theSetIdentifier) {
    ...
    CFCSET_VALIDATE_BUILTIN_TYPE(theSetIdentifier);

    CFSpinLock(&__CFBuiltinSetsLock);
    cset = (__CFBuiltinSets ? __CFBuiltinSets[theSetIdentifier - 1] : NULL);
    CFSpinUnlock(&__CFBuiltinSetsLock);

    if (cset) {
        return cset;
    }

    if (!(cset = __CFCSetGenericCreate(kCFAllocatorSystemDefault, __kCFCharSetClassBuiltin))) {
        return NULL;
    }
    __CFCSetPutBuiltinType(CF_CONST_CAST(CFMutableCharacterSetRef, cset), theSetIdentifier);

    CFSpinLock(&__CFBuiltinSetsLock);
    if (!__CFBuiltinSets) {
        __CFBuiltinSets = (CFCharacterSetRef*)CFAllocatorAllocate((CFAllocatorRef)CFRetain(CFAllocatorGetDefault()), sizeof(CFCharacterSetRef) * __kCFLastBuiltinSetID, 0);
...
CFCharacterSetRef CFCharacterSetCreateWithCharactersInRange(CFAllocatorRef allocator, CFRange theRange) {
    ...
    CFCSET_VALIDATE_RANGE(theRange);

    if (theRange.length) {
        if (!(cset = __CFCSetGenericCreate(allocator, __kCFCharSetClassRange))) {
            return NULL;
        }
        __CFCSetPutRangeFirstChar(cset, theRange.location);
        __CFCSetPutRangeLength(cset, theRange.length);
    } else {
        if (!(cset = __CFCSetGenericCreate(allocator, __kCFCharSetClassBitmap))) {
            return NULL;
        }
        __CFCSetPutBitmapBits(cset, NULL);
...
CFCharacterSetRef CFCharacterSetCreateWithCharactersInString(CFAllocatorRef allocator, CFStringRef theString) {
    ...
    if (length < __kCFStringCharSetMax) {
        ...
        if (!(cset = __CFCSetGenericCreate(allocator, __kCFCharSetClassString))) {
            return NULL;
        }
...
CFCharacterSetRef CFCharacterSetCreateInvertedSet(CFAllocatorRef alloc, CFCharacterSetRef theSet) {
    ...
    CF_OBJC_FUNCDISPATCH(CFCharacterSetRef, theSet, "invertedSet");
...
CFMutableCharacterSetRef CFCharacterSetCreateMutable(CFAllocatorRef allocator) {
    ...
    if (!(cset = __CFCSetGenericCreate(allocator, __kCFCharSetClassBitmap | __kCFCharSetIsMutable))) {
        return NULL;
    }
    __CFCSetPutBitmapBits(cset, NULL);
    __CFCSetPutHasHashValue(cset, true); // Hash value is 0
...
Boolean CFCharacterSetIsCharacterMember(CFCharacterSetRef theSet, UniChar theChar) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, theSet, "longCharacterIsMember:", theChar);
    ...
    switch (__CFCSetClassType(theSet)) {
        case __kCFCharSetClassBuiltin:
            result = (_CFUniCharIsMemberOf(theChar, __CFCSetBuiltinType(theSet)) ? !isInverted : isInverted);
            break;
        ...
        case __kCFCharSetClassString:
            result = __CFCSetBsearchUniChar(theSet, theChar) ? !isInverted : isInverted;
            break;

        case __kCFCharSetClassBitmap:
            result = (__CFCSetCompactBitmapBits(theSet) ? __CFCSetIsMemberBitmap(__CFCSetBitmapBits(theSet), theChar) : isInverted);
            break;

        case __kCFCharSetClassCompactBitmap:
            result = (__CFCSetCompactBitmapBits(theSet) ? __CFCSetIsMemberInCompactBitmap(__CFCSetCompactBitmapBits(theSet), theChar) : isInverted);
            break;
...
Boolean CFCharacterSetIsLongCharacterMember(CFCharacterSetRef theSet, UTF32Char theChar) {
    ...
    if (plane) {
        ...
        if (__CFCSetIsBuiltin(theSet)) {
            isInverted = __CFCSetIsInverted(theSet);
            return (_CFUniCharIsMemberOf(theChar, __CFCSetBuiltinType(theSet)) ? !isInverted : isInverted);
        }
...
Boolean CFCharacterSetIsSupersetOfSet(CFCharacterSetRef theSet, CFCharacterSetRef theOtherSet) {
    ...
    if ((!CF_IS_OBJC(theSet) || (expandedSet = __CFCharacterSetGetExpandedSetForNSCharacterSet(theSet))) && (!CF_IS_OBJC(theOtherSet) || (expandedOtherSet = __CFCharacterSetGetExpandedSetForNSCharacterSet(theOtherSet)))) { // Really CF, we can do some trick here
        if (expandedSet) {
            theSet = expandedSet;
        }
        if (expandedOtherSet) {
            theOtherSet = expandedOtherSet;
        }

        CF_VALIDATE_CSET_ARG(theSet);
        CF_VALIDATE_CSET_ARG(theOtherSet);
        ...
        if (__CFCSetIsEmpty(theSet)) {
            ...
            if (__CFCSetIsBuiltin(theSet) || __CFCSetIsBuiltin(theOtherSet)) {
                if (__CFCSetClassType(theSet) == __CFCSetClassType(theOtherSet) && __CFCSetBuiltinType(theSet) == __CFCSetBuiltinType(theOtherSet) && !__CFCSetIsInverted(theSet) && !__CFCSetIsInverted(theOtherSet)) {
                    return TRUE;
                }
                ...
                if (isTheSetAnnexInverted) {
                    theSetAnnexMask = (~theSetAnnexMask) & (0xFFFF < 1);
                }
                if (isTheOtherSetAnnexInverted) {
                    theOtherSetAnnexMask = (~theOtherSetAnnexMask) & (0xFFFF < 1);
                }

                __CFCSetGetBitmap(theSet, theSetBuffer);
                __CFCSetGetBitmap(theOtherSet, theOtherSetBuffer);

                if (!__CFCSetIsBitmapSupersetOfBitmap((const UInt32*)theSetBuffer, (const UInt32*)theOtherSetBuffer, FALSE, FALSE)) {
                    return FALSE;
                }
                ...
                if (theOtherSetAnnexMask) {
                    ...
                    if ((theSetAnnexMask & theOtherSetAnnexMask) != theOtherSetAnnexMask) {
                        return FALSE;
                    }
                    ...
                    for (idx = 1; idx <= 16; idx++) {
                        ...
                        if (!theOtherSetAnnex) {
                            if (isTheOtherSetAnnexInverted) {
                                ...
                                if (!__CFCSetIsEqualBitmap((const UInt32*)theSetBuffer, (isTheSetAnnexInverted ? NULL : (const UInt32*)-1))) {
                                    return FALSE;
                                }
                            }
                        } else {
                            ...
                            if (!__CFCSetIsBitmapSupersetOfBitmap((const UInt32*)theSetBuffer, (const UInt32*)theOtherSetBuffer, isTheSetAnnexInverted, isTheOtherSetAnnexInverted)) {
                                return FALSE;
                            }
                        }
                    }
    ...
    result = __CFCSetIsEqual(copy, theOtherSet);
    CFRelease(copy);
...
Boolean CFCharacterSetHasMemberInPlane(CFCharacterSetRef theSet, CFIndex thePlane) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, theSet, "hasMemberInPlane:", thePlane);
    ...
    if (__CFCSetIsEmpty(theSet)) {
        ...
        if (type == kCFCharacterSetControl) {
            if (isInverted || (thePlane == 14)) {
                ...
                return (_CFUniCharGetBitmapPtrForPlane(type, thePlane) ? TRUE : FALSE);
            }
        ...
        if (thePlane == 0) {
            switch (__CFCSetClassType(theSet)) {
                case __kCFCharSetClassString: if (!__CFCSetStringLength(theSet)) {
                    return isInverted;
                }
                    break;
...
CFDataRef CFCharacterSetCreateBitmapRepresentation(CFAllocatorRef alloc, CFCharacterSetRef theSet) {
    ...
    int planeIndices[__kCFMaxAnnexPlane];
    int idx;
    int length;
    bool isAnnexInverted;

    CF_OBJC_FUNCDISPATCH(CFDataRef, theSet, "_retainedBitmapRepresentation");
    ...
    if (__CFCSetHasNonBMPPlane(theSet)) {
        ...
        numNonBMPPlanes = (__CFCSetIsInverted(theSet) ? __kCFMaxAnnexPlane : _CFUniCharGetNumberOfPlanes(__CFCSetBuiltinType(theSet)) - 1);
    } else if (__CFCSetIsRange(theSet)) {
        ...
        if (lastPlane > 0) {
            ...
            if (isInverted) {
                numNonBMPPlanes = __kCFMaxAnnexPlane - numNonBMPPlanes;
                if (firstPlane == lastPlane) {
                    if (((firstChar & 0xFFFF) > 0) || ((lastChar & 0xFFFF) < 0xFFFF)) {
                        ++numNonBMPPlanes;
                    }
                } else {
                    if ((firstChar & 0xFFFF) > 0) {
                        ++numNonBMPPlanes;
                    }
                    if ((lastChar & 0xFFFF) < 0xFFFF) {
                        ++numNonBMPPlanes;
                    }
                }
            }
        } else if (isInverted) {
            numNonBMPPlanes = __kCFMaxAnnexPlane;
        }
    ...
    if (numNonBMPPlanes > 0) {
        ...
        if (__CFCSetHasNonBMPPlane(theSet)) {
            ...
            for (idx = 0; idx < numNonBMPPlanes; idx++) {
                if ((result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(theSet), idx + 1, bytes + 1,  (isInverted != 0))) == kCFUniCharBitmapEmpty) {
                    continue;
                }
                *(bytes++) = idx + 1;
                if (result == kCFUniCharBitmapAll) {
            ...
            if (__CFCSetIsInverted(theSet)) {
                ...
                for (idx = lastPlane + 1; idx <= __kCFMaxAnnexPlane; idx++) {
                    *(bytes++) = idx;
                    __CFCSetBitmapFastFillWithValue((UInt32*)bytes, 0xFF);
                    bytes += __kCFBitmapSize;
                }
...
void CFCharacterSetAddCharactersInRange(CFMutableCharacterSetRef theSet, CFRange theRange) {
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "addCharactersInRange:", theRange);

    CF_VALIDATE_MUTABLECSET_ARG(theSet);
    CFCSET_VALIDATE_RANGE(theRange);

    if (!theRange.length || (__CFCSetIsInverted(theSet) && __CFCSetIsEmpty(theSet))) {
    ...
    if (!__CFCSetIsInverted(theSet)) {
        if (__CFCSetIsEmpty(theSet)) {
            ...
            if (firstChar == theRange.location) {
                __CFCSetPutRangeLength(theSet, _CFMin(length, theRange.length));
                __CFCSetPutHasHashValue(theSet, false);
                return;
            } else if (firstChar < theRange.location && theRange.location <= firstChar + length) {
                if (firstChar + length < theRange.location + theRange.length) {
                    __CFCSetPutRangeLength(theSet, theRange.length + (theRange.location - firstChar));
                }
                __CFCSetPutHasHashValue(theSet, false);
                return;
            ...
            while (theRange.length--) {
                *buffer++ = (UniChar)theRange.location++;
            }
            __CFCSetSortUniChars(theSet);
            __CFCSetPutHasHashValue(theSet, false);
            return;
        }
    }
    ...
    if (theRange.location < 0x10000) { // theRange is in BMP
        if (theRange.location + theRange.length >= NUMCHARACTERS) {
            theRange.length = NUMCHARACTERS - theRange.location;
        }
    ...
    if (__CFCheckForExapendedSet) {
        __CFCheckForExpandedSet(theSet);
    }
}

void CFCharacterSetRemoveCharactersInRange(CFMutableCharacterSetRef theSet, CFRange theRange) {
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "removeCharactersInRange:", theRange);
...
void CFCharacterSetAddCharactersInString(CFMutableCharacterSetRef theSet,  CFStringRef theString) {
    ...
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "addCharactersInString:", theString);

    CF_VALIDATE_MUTABLECSET_ARG(theSet);

    if ((__CFCSetIsEmpty(theSet) && __CFCSetIsInverted(theSet)) || !(length = CFStringGetLength(theString))) {
        return;
    }
    ...
    if ((buffer = CFStringGetCharactersPtr(theString))) {
        while (length--) {
            __CFCSetBitmapAddCharacter(__CFCSetBitmapBits(theSet), *buffer++);
        }
    } else {
        ...
        for (idx = 0; idx < length; idx++) {
            __CFCSetBitmapAddCharacter(__CFCSetBitmapBits(theSet), CFStringGetCharacterFromInlineBuffer(&inlineBuffer, idx));
        }
    }
    __CFCSetPutHasHashValue(theSet, false);
    if (__CFCheckForExapendedSet) {
...
void CFCharacterSetRemoveCharactersInString(CFMutableCharacterSetRef theSet, CFStringRef theString) {
    ...
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "removeCharactersInString:", theString);
    ...
    if ((buffer = CFStringGetCharactersPtr(theString))) {
        while (length--) {
            __CFCSetBitmapRemoveCharacter(__CFCSetBitmapBits(theSet), *buffer++);
        }
    } else {
        CFStringInlineBuffer inlineBuffer;
        CFIndex idx;
        ...
        CFStringInitInlineBuffer(theString, &inlineBuffer, CFRangeMake(0, length));
        for (idx = 0; idx < length; idx++) {
            __CFCSetBitmapRemoveCharacter(__CFCSetBitmapBits(theSet), CFStringGetCharacterFromInlineBuffer(&inlineBuffer, idx));
        }
    }
    __CFCSetPutHasHashValue(theSet, false);
    if (__CFCheckForExapendedSet) {
...
void CFCharacterSetUnion(CFMutableCharacterSetRef theSet, CFCharacterSetRef theOtherSet) {
    ...
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "formUnionWithCharacterSet:", theOtherSet);

    CF_VALIDATE_MUTABLECSET_ARG(theSet);

    if (__CFCSetIsEmpty(theSet) && __CFCSetIsInverted(theSet)) {
        return;                                                        // Inverted empty set contains all char

    }
    if (!CF_IS_OBJC(theOtherSet) || (expandedSet = __CFCharacterSetGetExpandedSetForNSCharacterSet(theOtherSet))) { // Really CF, we can do some trick here
        if (expandedSet) {
            theOtherSet = expandedSet;
        }
        ...
        if (__CFCSetIsEmpty(theOtherSet)) {
            ...
            if (__CFCSetIsRange(theOtherSet)) {
                if (__CFCSetIsInverted(theOtherSet)) {
                    ...
                    if (firstChar > 0) {
                        CFCharacterSetAddCharactersInRange(theSet, CFRangeMake(0, firstChar));
                    }
                    firstChar += length;
            ...
            if (__CFCSetHasNonBMPPlane(theOtherSet)) {
                ...
                for (planeIndex = 1; planeIndex <= __kCFMaxAnnexPlane; planeIndex++) {
                    result = _CFUniCharGetBitmapForPlane(__CFCSetBuiltinType(theOtherSet), planeIndex, bitmapBuffer, (isOtherAnnexPlaneInverted != 0));
                    if (result != kCFUniCharBitmapEmpty) {
...
void CFCharacterSetIntersect(CFMutableCharacterSetRef theSet, CFCharacterSetRef theOtherSet) {
    ...
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "formIntersectionWithCharacterSet:", theOtherSet);
    ...
    if (!CF_IS_OBJC(theOtherSet) || (expandedSet = __CFCharacterSetGetExpandedSetForNSCharacterSet(theOtherSet))) { // Really CF, we can do some trick here
        ...
        if (__CFCSetIsEmpty(theOtherSet)) {
            ...
            switch (__CFCSetClassType(theOtherSet)) {
                ...
                default:
                    CF_REPORT_INVALID_CSET_TYPE(theOtherSet);
            }
        } else {
            __CFCSetMakeBitmap(theSet);
            if (__CFCSetIsBitmap(theOtherSet)) {
...
void CFCharacterSetInvert(CFMutableCharacterSetRef theSet) {
    ...
    CF_OBJC_VOID_FUNCDISPATCH(theSet, "invert");
...
void CFCharacterSetCompact(CFMutableCharacterSetRef theSet) {
    if (__CFCSetIsBitmap(theSet) && __CFCSetBitmapBits(theSet)) {
    ...
    if (__CFCSetHasNonBMPPlane(theSet)) {
        ...
        for (idx = 1; idx <= __kCFMaxAnnexPlane; idx++) {
            annex = (CFMutableCharacterSetRef)__CFCSetGetAnnexPlaneCharacterSetNoAlloc(theSet, idx);
            if (annex && __CFCSetIsBitmap(annex) && __CFCSetBitmapBits(annex)) {
...
void CFCharacterSetFast(CFMutableCharacterSetRef theSet) {
    if (__CFCSetIsCompactBitmap(theSet) && __CFCSetCompactBitmapBits(theSet)) {
        __CFCSetMakeBitmap(theSet);
    }
    if (__CFCSetHasNonBMPPlane(theSet)) {
        CFMutableCharacterSetRef annex;
        ...
        for (idx = 1; idx <= __kCFMaxAnnexPlane; idx++) {
            annex = (CFMutableCharacterSetRef)__CFCSetGetAnnexPlaneCharacterSetNoAlloc(theSet, idx);
            if (annex && __CFCSetIsCompactBitmap(annex) && __CFCSetCompactBitmapBits(annex)) {
...
void CFCharacterSetInitInlineBuffer(CFCharacterSetRef cset, CFCharacterSetInlineBuffer* buffer) {
    ...
    switch (__CFCSetClassType(cset)) {
        case __kCFCharSetClassBuiltin:
            buffer->bitmap = _CFUniCharGetBitmapPtrForPlane(__CFCSetBuiltinType(cset), 0);
            buffer->rangeLimit = 0x110000;
            if (!buffer->bitmap) {
                buffer->flags = kCFCharacterSetNoBitmapAvailable;
            } else {
                if (__CFCSetIsInverted(cset)) {
                    buffer->flags = kCFCharacterSetIsInverted;
                }
            }
            break;
        ...
        case __kCFCharSetClassCompactBitmap:
            ...
            if (!buffer->bitmap) {
                ...
                if (__kCFCharSetClassCompactBitmap == __CFCSetClassType(cset)) {
                    buffer->flags = kCFCharacterSetIsCompactBitmap;
                }
            }
            break;
        ...
        default:
            CF_REPORT_INVALID_CSET_TYPE(cset);
            return;
    }
    ...
    if (__CFCSetAnnexIsInverted(cset)) {
        ...
        for (index = __kCFMaxAnnexPlane; index > 0; index--) {
            if (__CFCSetGetAnnexPlaneCharacterSetNoAlloc(cset, index)) {
