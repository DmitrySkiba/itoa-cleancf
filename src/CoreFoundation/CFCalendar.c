...
#define CF_VALIDATE_CALENDAR_ARG(calendar) \
    CF_VALIDATE_OBJECT_ARG(CF, calendar, __kCFCalendarTypeID)
...
static void __CFCalendarSetupCal(CFCalendarRef calendar) {
    calendar->_cal = _CFCalendarCreateUCalendar(calendar->_identifier, calendar->_localeID, calendar->_tz);
}
...
static Boolean __validUnits(CFCalendarUnit smaller, CFCalendarUnit bigger) {
    switch (bigger) {
        case kCFCalendarUnitEra:
            if (kCFCalendarUnitEra == smaller) {
                return false;
            }
            if (kCFCalendarUnitWeekday == smaller) {
                return false;
            }
            if (kCFCalendarUnitMinute == smaller) {
                return false;                              // this causes CFIndex overflow in range.length
            }
            if (kCFCalendarUnitSecond == smaller) {
                return false;                              // this causes CFIndex overflow in range.length
            }
            return true;
        case kCFCalendarUnitYear:
            ...
            if (kCFCalendarUnitYear == smaller) {
                return false;
            }
            if (kCFCalendarUnitWeekday == smaller) {
                return false;
        ...
        case kCFCalendarUnitMonth:
            ...
            if (kCFCalendarUnitMonth == smaller) {
                return false;
            }
            if (kCFCalendarUnitWeekday == smaller) {
                return false;
            ...
            return true;
        case kCFCalendarUnitDay:
            if (kCFCalendarUnitHour == smaller) {
                return true;
            }
            if (kCFCalendarUnitMinute == smaller) {
                return true;
...
CF_INTERNAL void _CFCalendarInitialize(void) {
...
CFCalendarRef CFCalendarCreateWithIdentifier(CFAllocatorRef allocator, CFStringRef identifier) {
    ...
    CF_VALIDATE_PTR_ARG(identifier);
    if (identifier != kCFGregorianCalendar && identifier != kCFBuddhistCalendar && identifier != kCFJapaneseCalendar && identifier != kCFIslamicCalendar && identifier != kCFIslamicCivilCalendar && identifier != kCFHebrewCalendar && identifier != kCFChineseCalendar) {
        if (CFEqual(kCFGregorianCalendar, identifier)) {
            identifier = kCFGregorianCalendar;
        } else if (CFEqual(kCFBuddhistCalendar, identifier)) {
            identifier = kCFBuddhistCalendar;
        } else if (CFEqual(kCFJapaneseCalendar, identifier)) {
            identifier = kCFJapaneseCalendar;
        } else if (CFEqual(kCFIslamicCalendar, identifier)) {
            identifier = kCFIslamicCalendar;
        } else if (CFEqual(kCFIslamicCivilCalendar, identifier)) {
            identifier = kCFIslamicCivilCalendar;
        } else if (CFEqual(kCFHebrewCalendar, identifier)) {
            identifier = kCFHebrewCalendar;
        } else if (CFEqual(kCFChineseCalendar, identifier)) {
            identifier = kCFChineseCalendar;
        } else {return NULL; }
    }
...
CFStringRef CFCalendarGetIdentifier(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFStringRef, calendar, "calendarIdentifier");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    return calendar->_identifier;
}

CFLocaleRef CFCalendarCopyLocale(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFLocaleRef, calendar, "_copyLocale");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    return (CFLocaleRef)CFLocaleCreate(kCFAllocatorSystemDefault, calendar->_localeID);
}

void CFCalendarSetLocale(CFCalendarRef calendar, CFLocaleRef locale) {
    CF_OBJC_VOID_FUNCDISPATCH(calendar, "setLocale:", locale);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    CF_VALIDATE_PTR_ARG(locale);
...
CFTimeZoneRef CFCalendarCopyTimeZone(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFTimeZoneRef, calendar, "_copyTimeZone");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    return (CFTimeZoneRef)CFRetain(calendar->_tz);
}

void CFCalendarSetTimeZone(CFCalendarRef calendar, CFTimeZoneRef tz) {
    CF_OBJC_VOID_FUNCDISPATCH(calendar, "setTimeZone:", tz);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (tz != calendar->_tz) {
        CFRelease(calendar->_tz);
...
CFIndex CFCalendarGetFirstWeekday(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFIndex, calendar, "firstWeekday");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
...
void CFCalendarSetFirstWeekday(CFCalendarRef calendar, CFIndex wkdy) {
    CF_OBJC_VOID_FUNCDISPATCH(calendar, "setFirstWeekday:", wkdy);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
...
CFIndex CFCalendarGetMinimumDaysInFirstWeek(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFIndex, calendar, "minimumDaysInFirstWeek");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
...
void CFCalendarSetMinimumDaysInFirstWeek(CFCalendarRef calendar, CFIndex mwd) {
    CF_OBJC_VOID_FUNCDISPATCH(calendar, "setMinimumDaysInFirstWeek:", mwd);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
        ucal_setAttribute(calendar->_cal, UCAL_MINIMAL_DAYS_IN_FIRST_WEEK, mwd);
    }
}

CFDateRef CFCalendarCopyGregorianStartDate(CFCalendarRef calendar) {
    CF_OBJC_FUNCDISPATCH(CFDateRef, calendar, "_gregorianStartDate");
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
...
void CFCalendarSetGregorianStartDate(CFCalendarRef calendar, CFDateRef date) {
    CF_OBJC_VOID_FUNCDISPATCH(calendar, "_setGregorianStartDate:", date);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    ...
    if (!date) {
        UErrorCode status = U_ZERO_ERROR;
        UCalendar* cal = (UCalendar*)_CFCalendarCreateUCalendar(calendar->_identifier, calendar->_localeID, calendar->_tz);
        UDate udate = cal ? ucal_getGregorianChange(cal, &status) : 0;
        if (cal && U_SUCCESS(status)) {
            status = U_ZERO_ERROR;
            if (calendar->_cal) {
                ucal_setGregorianChange(calendar->_cal, udate, &status);
            }
        }
        if (cal) {
            ucal_close(cal);
...
CFRange CFCalendarGetMinimumRangeOfUnit(CFCalendarRef calendar, CFCalendarUnit unit) {
    //TODO _minimumRangeOfUnit:result:
    //CF_OBJC_FUNCDISPATCH(CFRange, calendar, "_minimumRangeOfUnit:", unit);
...
CFRange CFCalendarGetMaximumRangeOfUnit(CFCalendarRef calendar, CFCalendarUnit unit) {
    //TODO _maximumRangeOfUnit:result:
    //CF_OBJC_FUNCDISPATCH_STRET(CFRange, calendar, "_maximumRangeOfUnit:", unit);
    CFRange range = {kCFNotFound, kCFNotFound};
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
...
CFRange CFCalendarGetRangeOfUnit(CFCalendarRef calendar, CFCalendarUnit smallerUnit, CFCalendarUnit biggerUnit, CFAbsoluteTime at) {
    //TODO _rangeOfUnit:inUnit:forAT:result:
    //CF_OBJC_FUNCDISPATCH(CFRange, calendar, "_rangeOfUnit:inUnit:forAT:", smallerUnit, biggerUnit, at);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    CFRange range = {kCFNotFound, kCFNotFound};
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
 ...
 calculate:;
    ...
    for (;; ) {
        ...
        if (smallValue < minSmallValue) {
            minSmallValue = smallValue;
        }
        if (smallValue > maxSmallValue) {
            maxSmallValue = smallValue;
        }
        ucal_add(calendar->_cal, fieldToAdd, 1, &status);
        if (bigValue != ucal_get(calendar->_cal, bigField, &status)) {
            break;
        }
        if (biggerUnit == kCFCalendarUnitEra && ucal_get(calendar->_cal, yearField, &status) > 10000) {
            break;
        }
    ...
    if (smallerUnit == kCFCalendarUnitMonth) {
        range.location = 1;
    }
...
CFIndex CFCalendarGetOrdinalityOfUnit(CFCalendarRef calendar, CFCalendarUnit smallerUnit, CFCalendarUnit biggerUnit, CFAbsoluteTime at) {
    ...
    if (!__validUnits(smallerUnit, biggerUnit)) {
        return result;
    }
    CF_OBJC_FUNCDISPATCH(CFIndex, calendar, "_ordinalityOfUnit:inUnit:forAT:", smallerUnit, biggerUnit, at);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
        ...
        int multiple = (1 << multiple_table[_CFLastBitSet(smallerUnit) - 1]);
...
Boolean CFCalendarComposeAbsoluteTime(CFCalendarRef calendar, /* out */ CFAbsoluteTime* atp, const char* componentDesc, ...) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, calendar, "_composeAbsoluteTime:::", atp, componentDesc, args);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    
	size_t idx, cnt = strlen(componentDesc);
    _CF_ARRAY_ALLOCA(int, vector, cnt);
    ...
    va_end(args);
    return CFCalendarComposeAbsoluteTimeV(calendar, atp, componentDesc, vector);
}

Boolean CFCalendarComposeAbsoluteTimeV(CFCalendarRef calendar, /* out */ CFAbsoluteTime* atp, const char* componentDesc, int* vector) {
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
...
Boolean CFCalendarDecomposeAbsoluteTime(CFCalendarRef calendar, CFAbsoluteTime at, const char* componentDesc, ...) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, calendar, "_decomposeAbsoluteTime:::", at, componentDesc, args);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    ...
    va_end(args);
    return CFCalendarDecomposeAbsoluteTimeV(calendar, at, componentDesc, vector);
}

Boolean CFCalendarDecomposeAbsoluteTimeV(CFCalendarRef calendar, CFAbsoluteTime at, const char* componentDesc, int** vector) {
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
...
Boolean CFCalendarAddComponents(CFCalendarRef calendar, /* inout */ CFAbsoluteTime* atp, CFOptionFlags options, const char* componentDesc, ...) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, calendar, "_addComponents::::", atp, options, componentDesc, args);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    ...
    va_end(args);
    return CFCalendarAddComponentsV(calendar, atp, options, componentDesc, vector);
}

Boolean CFCalendarAddComponentsV(CFCalendarRef calendar, /*inout*/ CFAbsoluteTime* atp, CFOptionFlags options, const char* componentDesc, int* vector) {
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
...
Boolean CFCalendarGetComponentDifference(CFCalendarRef calendar, CFAbsoluteTime startingAT, CFAbsoluteTime resultAT, CFOptionFlags options, const char* componentDesc, ...) {
    ...
    CF_OBJC_FUNCDISPATCH(Boolean, calendar, "_diffComponents:::::", startingAT, resultAT, options, componentDesc, args);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    ...
    va_end(args);
    return CFCalendarGetComponentDifferenceV(calendar, startingAT, resultAT, options, componentDesc, vector);
}

Boolean CFCalendarGetComponentDifferenceV(CFCalendarRef calendar, CFAbsoluteTime startingAT, CFAbsoluteTime resultAT, CFOptionFlags options, const char* componentDesc, int** vector) {
    if (!calendar->_cal) {
        __CFCalendarSetupCal(calendar);
    }
    if (calendar->_cal) {
        ...
        while (ch) {
            UCalendarDateFields field = __CFCalendarGetICUFieldCodeFromChar(ch);
            const int multiple_table[] = {0, 0, 16, 19, 24, 26, 24, 28, 14, 14, 14};
            int multiple = direction * (1 << multiple_table[_CFLastBitSet(__CFCalendarGetCalendarUnitFromChar(ch)) - 1]);
            Boolean divide = false, alwaysDivide = false;
            int result = 0;
            while ((direction > 0 && curr < goal) || (direction < 0 && goal < curr)) {
                ...
                if ((direction > 0 && curr < newcurr && newcurr <= goal) || (direction < 0 && newcurr < curr && goal <= newcurr)) {
                    ...
                    // each multiple, so we reduce it every time through
                    // the loop.
                    if ((direction > 0 && goal < newcurr) || (direction < 0 && newcurr < goal)) {
                        alwaysDivide = true;
                    }
                }
                if (divide) {
...
Boolean CFCalendarGetTimeRangeOfUnit(CFCalendarRef calendar, CFCalendarUnit unit, CFAbsoluteTime at, CFAbsoluteTime* startp, CFTimeInterval* tip) {
    CF_OBJC_FUNCDISPATCH(Boolean, calendar, "_rangeOfUnit:startTime:interval:forAT:", unit, startp, tip, at);
    CF_VALIDATE_CALENDAR_ARG(calendar);
    if (kCFCalendarUnitWeekdayOrdinal == unit) {
        return false;
    }
    if (kCFCalendarUnitWeekday == unit) {
    ...
    if (calendar->_cal) {
        ...
        if (U_SUCCESS(status)) {
            if (startp) {
                *startp = (double)start / 1000.0 - kCFAbsoluteTimeIntervalSince1970;
            }
            if (tip) {
                *tip = (double)(end - start) / 1000.0;
            }
            return true;
        }
    }
