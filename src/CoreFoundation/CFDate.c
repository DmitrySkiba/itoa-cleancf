...
static const uint8_t __CFDaysInMonthTable[16] = {
    0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 0, 0, 0
};
static const uint16_t __CFDaysBeforeMonthTable[16] = {
    0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365, 0, 0
};
static const uint16_t __CFDaysAfterMonthTable[16] = {
    365, 334, 306, 275, 245, 214, 184, 153, 122, 92, 61, 31, 0, 0, 0, 0
};
...
CF_INLINE bool __CFIsLeapYear(int64_t year) {
    // Correct to nearest multiple-of-400 year, then find the remainder.
    int64_t y = (year + 1) % 400;
    if (y < 0) {
        y = -y;
    }
    return (!(y & 3) && 100 != y && 200 != y && 300 != y);
}

/* Year argument is absolute year;
 * Gregorian 2001 == year 0; 2001/1/1 = absolute date 0.
 */
CF_INLINE uint8_t __CFDaysInMonth(int8_t month, int64_t year) {
    bool leap = __CFIsLeapYear(year);
    return __CFDaysInMonthTable[month] + (2 == month && leap);
}

/* Year argument is absolute year;
 * Gregorian 2001 == year 0; 2001/1/1 = absolute date 0.
 */
CF_INLINE uint16_t __CFDaysBeforeMonth(int8_t month, int64_t year) {
    bool leap = __CFIsLeapYear(year);
    return __CFDaysBeforeMonthTable[month] + (2 < month && leap);
}

/* Year argument is absolute year;
 * Gregorian 2001 == year 0; 2001/1/1 = absolute date 0.
 */
CF_INLINE uint16_t __CFDaysAfterMonth(int8_t month, int64_t year) {
    bool leap = __CFIsLeapYear(year);
    return __CFDaysAfterMonthTable[month] + (month < 2 && leap);
}
...
static void __CFYMDFromAbsolute(int64_t absolute, int64_t* year, int8_t* month, int8_t* day) {
    ...
    if (month || day) {
        ...
        while (__CFDaysBeforeMonth(m + 1, y) <= absolute) {
            m++;
        }
        if (month) {
            *month = m;
        ...
        if (day) {
            *day = (uint8_t)(absolute - __CFDaysBeforeMonth(m, y) + 1);
        }
    }
}
...
CF_INTERNAL int64_t _CFTimeIntervalToTSR(CFTimeInterval ti) {
    if ((ti * __CFTSRRate) > INT64_MAX / 2) {
        return (INT64_MAX / 2);
    }
    return (int64_t)(ti * __CFTSRRate + 0.5);
}

CF_INTERNAL CFTimeInterval _CFTSRToTimeInterval(int64_t tsr) {
    return (CFTimeInterval)(tsr * __CF1_TSRRate);
}

CF_INTERNAL void _CFDateInitialize(void) {
    __CFTSRRate = CFPlatformGetTSRRatePerSecond();
...
CFTypeID CFDateGetTypeID(void) {
    if (_kCFRuntimeNotATypeID == __kCFDateTypeID) {
...
CFTimeInterval CFDateGetAbsoluteTime(CFDateRef date) {
    //TODO _cfTimeIntervalSinceReferenceDate:result:
    //CF_OBJC_FUNCDISPATCH(CFTimeInterval, date, "timeIntervalSinceReferenceDate");
...
CFTimeInterval CFDateGetTimeIntervalSinceDate(CFDateRef date, CFDateRef otherDate) {
    //TODO _cfTimeIntervalSinceDate:result:
    //CF_OBJC_FUNCDISPATCH(CFTimeInterval, date, "timeIntervalSinceDate:", otherDate);
    CF_VALIDATE_OBJECT_ARG(CF, date, CFDateGetTypeID());
    CF_VALIDATE_OBJECT_ARG(CF, otherDate, CFDateGetTypeID());
    return date->_time - otherDate->_time;
}

CFComparisonResult CFDateCompare(CFDateRef date, CFDateRef otherDate, void* context) {
    CF_OBJC_FUNCDISPATCH(CFComparisonResult, date, "compare:", otherDate);
    CF_VALIDATE_OBJECT_ARG(CF, date, CFDateGetTypeID());
    CF_VALIDATE_OBJECT_ARG(CF, otherDate, CFDateGetTypeID());
    if (date->_time < otherDate->_time) {
...
Boolean CFGregorianDateIsValid(CFGregorianDate gdate, CFOptionFlags unitFlags) {
    ...
    if ((unitFlags & kCFGregorianUnitsMonths) &&
        (gdate.month < 1 || 12 < gdate.month))
    {
        return false;
    }
    if ((unitFlags & kCFGregorianUnitsDays) &&
        (gdate.day < 1 || 31 < gdate.day))
    {
        return false;
    }
    if ((unitFlags & kCFGregorianUnitsHours) &&
        (gdate.hour < 0 || 23 < gdate.hour))
    {
        return false;
    }
    if ((unitFlags & kCFGregorianUnitsMinutes) &&
        (gdate.minute < 0 || 59 < gdate.minute))
    {
        return false;
    }
    if ((unitFlags & kCFGregorianUnitsSeconds) &&
        (gdate.second < 0.0 || 60.0 <= gdate.second))
    {
        return false;
    }
    if ((unitFlags & kCFGregorianUnitsDays) &&
        (unitFlags & kCFGregorianUnitsMonths) &&
        (unitFlags & kCFGregorianUnitsYears) &&
        __CFDaysInMonth(gdate.month, gdate.year - 2001) < gdate.day)
    {
        return false;
    }
    return true;
