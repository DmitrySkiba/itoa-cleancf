...
enum {
    ...
    NSCFEncodingTag = 0x80000000
};

enum {
    MacCodepageBase = 10000,
    ISO8859CodepageBase = 28590
};
...
// TODO create struct __CFACPToCFEncodingEntry and rewrite those
//  dreadful raw accesses.
static SInt32 __CFACPToEncoding(uint16_t target) {
    const unsigned int* start, * end, * divider;
    unsigned int size = sizeof(__CFACPToCFEncodingTable) / sizeof(UInt32);

    start = (const unsigned int*)__CFACPToCFEncodingTable;
    end = (const unsigned int*)__CFACPToCFEncodingTable + (size - 1);
    while (start <= end) {
        divider = start + ((end - start) / 2);

        if (*(const uint16_t*)divider == target) {
            return *((const uint16_t*)divider + 1);
        } else if (*(const uint16_t*)divider > target) {
            end = divider - 1;
        } else if (*(const uint16_t*)(divider + 1) > target) {
            return *((const uint16_t*)divider + 1);
...
CFStringRef CFStringGetNameOfEncoding(CFStringEncoding theEncoding) {
    ...
    CFStringRef theName = mappingTable ? 
        (CFStringRef)CFDictionaryGetValue(mappingTable, CF_CAST(void*, theEncoding)) :
        NULL;
    ...
    if (!theName) {
        switch (theEncoding) {
            ...
            default:
            {
                ...
                if (encodingName) {
                    theName = CFStringCreateWithCString(
                        kCFAllocatorSystemDefault, 
                        encodingName, kCFStringEncodingASCII);
                }
                break;
            }
        }
        ...
        if (theName) {
            if (!mappingTable) {
                mappingTable = CFDictionaryCreateMutable(
                    kCFAllocatorSystemDefault,
                    0,
                    NULL,
                    &kCFTypeDictionaryValueCallBacks);
            }
            CFDictionaryAddValue(mappingTable, CF_CAST(void*, theEncoding), theName);
            CFRelease(theName);
        }
    }
...
CFStringEncoding CFStringConvertIANACharSetNameToEncoding(CFStringRef charsetName) {
    ...
    if (!mappingTable) {
            ...
            if (nameList) {
                while (*nameList) {
                    CFStringRef name = CFStringCreateWithCString(
                        kCFAllocatorSystemDefault,
                        *nameList++, kCFStringEncodingASCII);
                    if (name) {
                        CFDictionaryAddValue(table, name, CF_CAST(void*, *encodings));
                        CFRelease(name);
                    }
                }
            }
        ...
        CFDictionaryAddValue(table, CFSTR("unicode-1-1"), (void*)kCFStringEncodingUTF16);
        CFDictionaryAddValue(table, CFSTR("iso-10646-ucs-2"), (void*)kCFStringEncodingUTF16);
        CFDictionaryAddValue(table, CFSTR("utf-16"), (void*)kCFStringEncodingUTF16);
        CFDictionaryAddValue(table, CFSTR("utf-16be"), (void*)kCFStringEncodingUTF16BE);
        CFDictionaryAddValue(table, CFSTR("utf-16le"), (void*)kCFStringEncodingUTF16LE);
        CFDictionaryAddValue(table, CFSTR("utf-32"), (void*)kCFStringEncodingUTF32);
        CFDictionaryAddValue(table, CFSTR("utf-32be"), (void*)kCFStringEncodingUTF32BE);
        CFDictionaryAddValue(table, CFSTR("utf-32le"), (void*)kCFStringEncodingUTF32LE);

        mappingTable = table;
    }

    if (CFDictionaryContainsKey(mappingTable, lowerCharsetName)) {
        result = CF_CAST(CFStringEncoding, CFDictionaryGetValue(mappingTable, lowerCharsetName));
    }
...
CFStringRef CFStringConvertEncodingToIANACharSetName(CFStringEncoding encoding) {
    static CFMutableDictionaryRef mappingTable = NULL;
    CFStringRef theName = mappingTable ? 
        (CFStringRef)CFDictionaryGetValue(mappingTable, CF_CAST(void*, encoding)) :
        NULL;
    ...
    if (!theName) {
            ...
            default:
            {
                ...
                if (nameList && *nameList) {
                    theName = CFStringCreateWithCString(
                        ...
                        *nameList, kCFStringEncodingASCII);
                    if (theName) {
                        CFMutableStringRef upperCaseName = CFStringCreateMutableCopy(
                            kCFAllocatorSystemDefault,
                            0,
                            theName);
                        CFStringUppercase(upperCaseName, NULL);
                        CFRelease(theName);
        ...
        if (theName) {
            ...
            CFDictionaryAddValue(mappingTable, CF_CAST(void*, encoding), theName);
            CFRelease(theName);
        }
    }
...
unsigned long CFStringConvertEncodingToNSStringEncoding(CFStringEncoding theEncoding) {
    switch (theEncoding & 0xFFF) {
        ...
        case kCFStringEncodingUnicode:
            if (theEncoding == kCFStringEncodingUTF16) {
        ...
        default:
            return NSCFEncodingTag | theEncoding;
    }
}
...
CFStringEncoding CFStringConvertNSStringEncodingToEncoding(unsigned long theEncoding) {
    switch (theEncoding) {
        ...
        default:
            return (theEncoding & NSCFEncodingTag) ? 
                (theEncoding & ~NSCFEncodingTag) :
...
UInt32 CFStringConvertEncodingToWindowsCodepage(CFStringEncoding theEncoding) {
    ...
    switch (theEncoding & 0x0F00) {
        ...
        case 0x0200: // ISO 8859 series
            if (encodingBase <= kCFStringEncodingISOLatin10) {
                return ISO8859CodepageBase + (encodingBase - 0x200);
            }
            break;

        case 0x0400: // DOS codepage
            if (encodingBase <= kCFStringEncodingDOSChineseTrad) {
                return _CFToDOSCodePageList[encodingBase - 0x400];
            }
            break;

        case 0x0500: // ANSI (Windows) codepage
            if (encodingBase <= kCFStringEncodingWindowsVietnamese) {
                return _CFToWindowsCodePageList[theEncoding - 0x500];
            } else if (encodingBase == kCFStringEncodingWindowsKoreanJohab) {
                return 1361;
            }
            break;

        case 0x600: // National standards
            if (encodingBase == kCFStringEncodingASCII) {
                return 20127;
            } else if (encodingBase == kCFStringEncodingGB_18030_2000) {
                return 54936;
            }
            break;
        ...
        case 0x0900: // EUC series
            if (encodingBase <= kCFStringEncodingEUC_KR) {
                return _CFEUCToCodePage[encodingBase - 0x0900];
            }
            break;
        ...
        case 0x0C00: // IBM EBCDIC encodings
            if (encodingBase == kCFStringEncodingEBCDIC_CP037) {
                return 37;
            }
            break;
    }
...
CFStringEncoding CFStringConvertWindowsCodepageToEncoding(UInt32 theEncoding) {
    if (theEncoding == 0 || theEncoding == 1) {
        // ID for default (system) codepage
        return CFStringGetSystemEncoding();
    } else if ((theEncoding >= MacCodepageBase) && (theEncoding < 20000)) {
        // Mac script
        if (theEncoding <= 10029) {
            // up to Mac Central European
            return theEncoding - MacCodepageBase;
        }
    } else if ((theEncoding >= ISO8859CodepageBase) && (theEncoding <= 28605)) {
        // ISO 8859
        return (theEncoding - ISO8859CodepageBase) + 0x200;
    } else if (theEncoding == 65001) {
        // UTF-8
        return kCFStringEncodingUTF8;
    } else if (theEncoding == 12000) {
        // UTF-16
        return kCFStringEncodingUTF16;
    } else if (theEncoding == 12001) {
        // UTF-16BE
        return kCFStringEncodingUTF16BE;
    } else if (theEncoding == 65005) {
        // UTF-32
        return kCFStringEncodingUTF32;
    } else if (theEncoding == 65006) {
        // UTF-32BE
        return kCFStringEncodingUTF32BE;
    } else {
        return __CFACPToEncoding(theEncoding);
    }
...
CFStringEncoding CFStringGetMostCompatibleMacStringEncoding(CFStringEncoding encoding) {
    return CFStringEncodingGetScriptCodeForEncoding(encoding);
}
